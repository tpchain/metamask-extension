...
const REQUESTS_STORE_KEY = 'pendingApprovals'
const COUNT_STORE_KEY = 'pendingApprovalsCount'
...
export default class ApprovalController {
  ...
  constructor ({ showApprovalRequest } = {}) {
    ...
    this.store = new ObservableStore({
      [REQUESTS_STORE_KEY]: {},
      [COUNT_STORE_KEY]: this.approvalCount,
    })
  }
   ...
   */
  get (id) {
    const info = this.store.getState()[REQUESTS_STORE_KEY][id]
    return info
      ? { ...info }
      : undefined
  }
  ...
  clear () {
    ...
    this._origins = {}
    this.approvalCount = 0
    this.store.putState({ [REQUESTS_STORE_KEY]: {}, [COUNT_STORE_KEY]: 0 })
  }
  ...
  _delete (id) {
    ...
    if (this._approvals.has(id)) {
      ...
      const state = this.store.getState()[REQUESTS_STORE_KEY]
      const {
        origin,
        type = DEFAULT_TYPE,
      } = state[id] || {}
